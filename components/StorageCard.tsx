...
export function StorageCard({ data }: { data: QuotaResponse & { label: string } }) {
  if (!data.success) {
    return (
      <div data-storage-card="loaded" className="rounded-xl border border-red/30 bg-panel p-4 shadow-card">
  ...
  return (
    <div data-storage-card="loaded" className="rounded-xl border border-border bg-panel p-4 shadow-card">
